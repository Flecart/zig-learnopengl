...
pub fn main() !void {
    ...
    while (!window.shouldClose()) {
        ...
        const radius = 10.0;
        const camX = @as(f32, @floatCast(@sin(glfw.getTime()))) * radius;
        const camZ = @as(f32, @floatCast(@cos(glfw.getTime()))) * radius;
        const view = math.Mat4.createLookAt(
            math.vec3(camX, 0, camZ),
